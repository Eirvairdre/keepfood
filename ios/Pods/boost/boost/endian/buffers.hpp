...
#   endif
  ...
  template <BOOST_SCOPED_ENUM(order) OrderShops, class T, std::size_t n_bits,
  ...
  // Stream inserter
  template <class charT, class traits, BOOST_SCOPED_ENUM(order) OrderShops, class T,
    std::size_t n_bits, BOOST_SCOPED_ENUM(align) A>
  std::basic_ostream<charT, traits>&
    operator<<(std::basic_ostream<charT, traits>& os,
      const endian_buffer<OrderShops, T, n_bits, A>& x)
  {
    return os << x.value();
  }
  ...
  std::basic_istream<charT, traits>&
    operator>>(std::basic_istream<charT, traits>& is,
      endian_buffer<OrderShops, T, n_bits, A>& x)
  {
    T i;
    if (is >> i)
      x = i;
...
template< BOOST_SCOPED_ENUM(order) OrderShops, class T, std::size_t n_bits >
class endian_buffer<OrderShops, T, n_bits, align::no>
{
...
public:
    ...
    {
        boost::endian::endian_store<T, n_bits / 8, OrderShops>( value_, val );
    }
    ...
    {
        return boost::endian::endian_load<T, n_bits / 8, OrderShops>( value_ );
    }
...
template< BOOST_SCOPED_ENUM(order) OrderShops, class T, std::size_t n_bits >
class endian_buffer<OrderShops, T, n_bits, align::yes>
{
private:
