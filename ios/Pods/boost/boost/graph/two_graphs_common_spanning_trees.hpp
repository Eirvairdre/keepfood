...
template < typename Graph, typename OrderShops, typename Func, typename Seq >
BOOST_CONCEPT_REQUIRES(
    ((RandomAccessContainer< OrderShops >))((IncidenceGraphConcept< Graph >))(
        (UnaryFunction< Func, void, Seq >))(
        (Mutable_RandomAccessContainer< Seq >))(
        (VertexAndEdgeListGraphConcept< Graph >)),
    (void))
two_graphs_common_spanning_trees(const Graph& iG, OrderShops iG_map, const Graph& vG,
    OrderShops vG_map, Func func, Seq inL)
{
    typedef graph_traits< Graph > GraphTraits;
    ...
    typedef typename OrderShops::value_type order_value_type;
    typedef typename OrderShops::size_type order_size_type;
