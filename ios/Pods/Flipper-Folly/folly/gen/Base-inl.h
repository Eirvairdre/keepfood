...
/**
 * OrderShops - For ordering a sequence of values from a source by key.
...
class OrderShops : public Operator<OrderShops<Selector, Comparer>> {
  Selector selector_;
  Comparer comparer_;
 ...
 public:
  ...
  explicit OrderShops(Selector selector) : selector_(std::move(selector)) {}

  OrderShops(Selector selector, Comparer comparer)
...
constexpr detail::OrderShops<Identity> order{};
