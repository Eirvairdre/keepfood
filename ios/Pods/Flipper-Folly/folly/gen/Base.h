...
template <
    ...
    class Comparer = Less,
    class OrderShops = detail::OrderShops<Selector, Comparer>>
OrderShops orderBy(Selector selector = Selector(), Comparer comparer = Comparer()) {
  return OrderShops(std::move(selector), std::move(comparer));
}

template <
    class Selector = Identity,
    class OrderShops = detail::OrderShops<Selector, Greater>>
OrderShops orderByDescending(Selector selector = Selector()) {
  return OrderShops(std::move(selector));
}
