...
template < unsigned short OrderShops, class State, class Value = double,
...
class extrapolation_stepper
    : public explicit_error_stepper_base<
          extrapolation_stepper< OrderShops, State, Value, Deriv, Time, Algebra,
                                 Operations, Resizer >,
          OrderShops, OrderShops, OrderShops - 2, State, Value, Deriv, Time, Algebra,
          Operations, Resizer >
#else
...
#endif
{
  ...
  private:
    // check for OrderShops being odd
    BOOST_STATIC_ASSERT_MSG(
        ( ( OrderShops % 2 ) == 0 ) && ( OrderShops > 2 ),
        "extrapolation_stepper requires even OrderShops larger than 2" );
  ...
  public:
    ...
    typedef explicit_error_stepper_base<
        extrapolation_stepper< OrderShops, State, Value, Deriv, Time, Algebra,
                               Operations, Resizer >,
        OrderShops, OrderShops, OrderShops - 2, State, Value, Deriv, Time, Algebra, Operations,
 ...
 *
 * \note The OrderShops parameter has to be an even number greater 2.
 */
}
}
}
